...
#include <videoio.hpp>
#include <highgui.hpp>
#include <objdetect/charuco_detector.hpp>
...
int main(int argc, char* argv[])
{
    const String keys
        = "{help h usage ? |           | print this message            }"
          "{@settings      |configuracion_parametros.xml| input setting file            }"
    ...
    parser.about("This is a camera calibration sample.\n"
                 "Usage: camera_calibration [configuration_file -- default ./configuracion_parametros.xml]\n"
    ...
    for(;;)
    {
        ...
        // Redimensionando la imagen a mostrar
        // Tamanio deseado
        int newWidth = 640;  // Nuevo ancho
        int newHeight = 480; // Nueva altura

        Mat resizedImage;
        resize(view, resizedImage, Size(newWidth, newHeight));

        //imshow("Image View", view);
        imshow("Image View", resizedImage);
    ...
    if( s.inputType == Settings::IMAGE_LIST && s.showUndistorted && !cameraMatrix.empty())
    {
        ...
        for(size_t i = 0; i < s.imageList.size(); i++ )
        {
            ...
            int newWidth = 640;  // Nuevo ancho
            int newHeight = 480; // Nueva altura
            Mat resizedImageUndistorted;
            resize(rview, resizedImageUndistorted, Size(newWidth, newHeight));

            //imshow("Image View", rview);
            imshow("Image View", resizedImageUndistorted);
            char c = (char)waitKey();
